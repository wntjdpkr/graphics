...
public:
	...
	XMMATRIX GetBaseViewMatrix() const { return m_baseViewMatrix; }

	bool SetFPS(int, ID3D11DeviceContext*);
	bool SetCPU(int, ID3D11DeviceContext*);
	bool SetPolyCount(int count, ID3D11DeviceContext* deviceContext);
	bool SetResolution(int width, int height, ID3D11DeviceContext* deviceContext);
	bool SetMenuText(int index, const char* text, int posX, int posY, float r, float g, float b, ID3D11DeviceContext*);
	bool SetModelCount(int modelCount, ID3D11DeviceContext* deviceContext);
