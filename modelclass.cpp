...
// (2) 인스턴스 버퍼 초기화 함수 구현
bool ModelClass::InitializeInstanceBuffer(ID3D11Device* device, const XMMATRIX* instanceData, int instanceCount)
{
	D3D11_BUFFER_DESC desc{};
	desc.Usage = D3D11_USAGE_DEFAULT;
	desc.ByteWidth = sizeof(XMMATRIX) * instanceCount;
	...
	HRESULT hr = device->CreateBuffer(&desc, &initData, &m_instanceBuffer);
	return SUCCEEDED(hr);
}
...
void ModelClass::ShutdownInstanceBuffer()
{
	if (m_instanceBuffer)
	{
...
// (3) RenderBuffers()를 RenderInstancedBuffers()로 분리
void ModelClass::RenderInstancedBuffers(ID3D11DeviceContext* deviceContext, int instanceCount)
{
	// 기존 정점/인덱스 바인딩
	UINT strides[2] = { sizeof(VertexType), sizeof(XMMATRIX) };
	UINT offsets[2] = { 0, 0 };
