...
public:
	...
	bool Initialize(ID3D11Device*, HWND, bool useInstancing);
	void Shutdown();
	bool Render(ID3D11DeviceContext*, int, XMMATRIX, XMMATRIX, XMMATRIX, ID3D11ShaderResourceView*);
	bool RenderInstanced(ID3D11DeviceContext* deviceContext, int indexCount, int instanceCount,
		XMMATRIX worldMatrix, XMMATRIX viewMatrix, XMMATRIX projectionMatrix, ID3D11ShaderResourceView * texture);
	void SetFilterMode(int mode);

private:
	bool InitializeShader(ID3D11Device*, HWND, const WCHAR*, bool useInstancing);
	void ShutdownShader();
